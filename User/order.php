<?php
 include 'components/connection.php'; 
    ...
    if(isset($_POST['delete_item'])){
        ...
        $orders_id = filter_var($orders_id, FILTER_SANITIZE_STRING);

        $varify_orders = $conn->prepare("SELECT * FROM orders WHERE id = ?");
        $varify_orders->execute([$orders_id]);
        
        if($varify_orders->rowCount() > 0){
            $delete_orders = $conn->prepare("DELETE FROM orders WHERE id = ?");
            $delete_orders->execute([$orders_id]);
            $message_msg[] = "Product removed from orders";
        }else{
            $message_msg[] = "orders item already deleted";
        }
...
<body> 
    ...
    <div class="main">
        ...
        <section class= "products">
            ...
            // phân trang sản phẩm trong orders
                ...
                if ($select_products->rowCount() > 0) {
                    while ($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)) {
                        // Xử lý hiển thị sản phẩm trong orders
                        $grand_total = 0;
            ...
            <?php 
                    $grand_total += $fetch_orders['price'];
                                }
                            }
                        }
                    }
            ...
            <div class="pagination">
                <?php  
                    ...
                    // $total_products_sql = "SELECT COUNT(*) AS total_products FROM orders WHERE 1=1";
